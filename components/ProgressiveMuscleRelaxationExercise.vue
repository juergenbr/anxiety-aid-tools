<template>
  <section class="sektion" ref="exerciseSection">
    ...
    <div v-if="!exerciseStarted && !exerciseCompleted" class="text-center">
      <div class="mb-8">
        ...
        >
          ...
          <span>{{ $t('exercises.progressiveMuscleRelaxation.interface.beginExercise') }}</span>
        </button>
      </div>
    </div>
    ...
    <div v-if="exerciseStarted && !exerciseCompleted">
      ...
      <SessionHeader
        icon="ph:user-focus"
        :title="$t('exercises.progressiveMuscleRelaxation.interface.title')"
        :subtitle="$t('exercises.progressiveMuscleRelaxation.interface.subtitle')"
        :display-value="currentGroupIndex + 1"
        :display-label="$t('exercises.progressiveMuscleRelaxation.interface.ofGroups', { count: muscleGroups.length })"
        :progress="((currentGroupIndex + 1) / muscleGroups.length) * 100"
        :status-text="currentState === 'tense' ? $t('exercises.progressiveMuscleRelaxation.interface.tenseFor5') : currentState === 'relax' ? $t('exercises.progressiveMuscleRelaxation.interface.releaseFor15') : $t('exercises.progressiveMuscleRelaxation.interface.prepare')"
        ...
        <div class="flex flex-col items-center">
          ...
          <div class="mb-6 text-center">
            ...
            <p v-else-if="currentState === 'tense'" class="mb-3 text-2xl font-light text-gray-700">
              {{ $t('exercises.progressiveMuscleRelaxation.interface.tense') }}
            </p>
            <p v-else-if="currentState === 'relax'" class="mb-3 text-2xl font-light text-gray-700">
              {{ $t('exercises.progressiveMuscleRelaxation.interface.relax') }}
            </p>
          ...
          <div class="mb-6 w-full max-w-sm">
            <div v-if="currentState === 'tense'" class="bg-red-50 p-4">
              <p class="mb-3 font-semibold text-red-800">{{ $t('exercises.progressiveMuscleRelaxation.interface.tensionSteps') }}</p>
            ...
            <div v-if="currentState === 'relax'" class="bg-green-50 p-4">
              <p class="mb-3 font-semibold text-green-800">{{ $t('exercises.progressiveMuscleRelaxation.interface.relaxationSteps') }}</p>
          ...
          <div class="mb-6 w-full max-w-sm">
            ...
            <div class="mb-4">
              <div class="mb-2 flex items-center justify-between">
                <span class="text-sm font-medium text-gray-700">{{ $t('exercises.progressiveMuscleRelaxation.interface.sessionProgress') }}</span>
          ...
          <div class="flex gap-3">
            ...
            >
              ...
              <span>{{ $t('exercises.progressiveMuscleRelaxation.interface.start') }}</span>
            </button>
            ...
            >
              ...
              <span>{{ $t('exercises.progressiveMuscleRelaxation.interface.skip') }}</span>
            </button>
            ...
            >
              ...
              <span>{{ $t('exercises.progressiveMuscleRelaxation.interface.stop') }}</span>
            </button>
          </div>
        </div>
      </div>
    ...
    <div v-if="exerciseCompleted" class="mb-6 border border-green-200 bg-green-50 p-8 text-center">
      ...
      <h2 class="mb-2 text-xl font-semibold text-gray-800">{{ $t('exercises.progressiveMuscleRelaxation.completion.title') }}</h2>
      <p class="mb-4 text-gray-600">
        {{ $t('exercises.progressiveMuscleRelaxation.completion.description') }}
      </p>
      <div class="flex items-center justify-center gap-4 text-sm text-gray-500">
        <div class="flex items-center gap-1">
          <Icon name="ph:lightning-slash" class="text-blue-400" />
          <span>{{ $t('exercises.progressiveMuscleRelaxation.completion.benefits.tensionReleased') }}</span>
        </div>
        <div class="flex items-center gap-1">
          <Icon name="ph:heart" class="text-red-400" />
          <span>{{ $t('exercises.progressiveMuscleRelaxation.completion.benefits.bodyRelaxed') }}</span>
        </div>
      </div>
    </div>
  </section>
